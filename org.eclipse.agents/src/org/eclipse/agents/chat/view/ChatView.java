...
import org.eclipse.agents.chat.view.ContentAssistProvider.ResourceProposal;
...
public class ChatView extends ViewPart implements TraverseListener, IContentProposalListener, ModifyListener, VerifyListener, Listener  {
	...
	public static final String ID  = "org.eclipse.agents.chat.view.ChatView"; //$NON-NLS-1$

	Text inputText;
	boolean disposed = false;
	ChatAddedResources contexts;
	ChatBrowser browser;
...
//		PlatformUI.getWorkbench().getHelpSystem().setHelp(parent, IConsoleHelpContextIds.CONSOLE_VIEW);
		...
		contexts = new ChatAddedResources(middle, SWT.NONE);
	...
	public void startPromptTurn() {
		activeSessionId = AgentClientController.instance().getActiveSessionId();
		if (agentConnected && activeSessionId != null) {
			...
			AgentClientController.instance().prompt(activeSessionId, content.toArray(ContentBlock[]::new));
	...
	public void stopPromptTurn() {
		AgentClientController.instance().stopPromptTurn(activeSessionId);
	}
