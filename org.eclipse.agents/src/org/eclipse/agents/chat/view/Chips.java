//TODO can we use official Nebula Chip instead
...
public class Chips extends Canvas {
	...
	private final ListenerList<CloseListener> closeListeners = new ListenerList<CloseListener>();
	private final ListenerList<SelectionListener> selectionListeners = new ListenerList<SelectionListener>();
	...
	public Chips(final Composite parent, final int style) {
		...
		addListener(SWT.MouseUp, e -> {
			...
			for (final SelectionListener listener : selectionListeners) {
				...
				event.button = e==null?1:e.button;
				event.display = this.getDisplay();
				event.item = null;
				event.widget = this;
				event.data = e == null ? null : e.data;
				event.time = e == null ? 0 : e.time;
				event.x = e == null ? 0 : e.x;
				event.y = e == null ? 0 : e.y;
	...
	private void initDefaultColors() {
		...
		setForeground(colorRegistry.get("CONTENT_ASSIST_FOREGROUND_COLOR"));
		chipsBackground = colorRegistry.get("CONTENT_ASSIST_BACKGROUND_COLOR");
//
		hoverForeground = getForeground();
		hoverBackground = getBackground();

		closeButtonForeground = getForeground();
		closeButtonBackground = getBackground();
		closeButtonHoverForeground = getForeground();
		closeButtonHoverBackground = getBackground();

		pushedStateForeground =getForeground();
		pushedStateBackground = getBackground();
	}
